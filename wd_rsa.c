...
static int wd_rsa_open_driver(int init_type)
{
	...
	wd_rsa_setting.adapter->workers[0].driver = driver;
...
int wd_rsa_init2_(char *alg, __u32 sched_type, int task_type, struct wd_ctx_params *ctx_params)
{
	...
	do {
		...
		worker->driver = drv;
       		...
       		ret = wd_ctx_param_init(&rsa_ctx_params, ctx_params, drv,
...
void wd_rsa_free_sess(handle_t h_sess)
{
	struct wd_rsa_sess *sess = (struct wd_rsa_sess *)h_sess;
...
{
	wd_alg_env_uninit(&wd_rsa_env_config, &wd_rsa_ops);
}
