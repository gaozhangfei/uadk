...
/* no work */
void *wd_blkpool_va(void *pool, void *pa)
{
	struct wd_blkpool *p = pool;
	struct wd_blk_hd *hd;
	...
	TAILQ_FOREACH(hd, &p->head, next) {
		if ((uintptr_t)pa >= (uintptr_t)hd->blk_dma &&
		    (uintptr_t)pa < (uintptr_t)hd->blk_dma + p->blk_sz) {
			return (void *)((uintptr_t)hd->blk +
...
#define ADDR_ALIGN_64(addr) (((uintptr_t)(addr) + 63) & ~63)
handle_t wd_blkpool_create_sglpool(void *pool, __u32 sgl_num, __u32 sge_num)
{
	...
	int sgl_size, size, i, j, blk_num;
	__u32 num = 0, num_per_blk;
	void *base;

	if (!sge_num || sge_num > HISI_SGE_NUM_IN_SGL ||
	    !sgl_num || sgl_num > HISI_SGL_NUM_IN_BD) {
	...
	sgl_size = sizeof(struct hisi_sgl) + sge_num * sizeof(struct hisi_sge);
	sgl_size = ALIGN(sgl_size, 64); /* 64 bytes aligned */
	num_per_blk = p->blk_sz / sgl_size;
	size = num_per_blk * sgl_size;
	blk_num = (sgl_num + num_per_blk - 1) / num_per_blk;
	sgl_pool->blk_num = blk_num;

	sgl_pool->blks = calloc(blk_num, sizeof(void *));
	if (!sgl_pool->blks) {
		WD_ERR("failed to alloc memory for sgl blks!\n");
		goto err_out;
	}

	for (i = 0; i < blk_num; i++) {
		base = wd_blkpool_alloc(p, size);
		if (!base) {
			WD_ERR("blk_pool_alloc failed!\n");
			goto err_out;
		}
		sgl_pool->blks[i] = base;

		for (j = 0; j < num_per_blk; j++) {
			sgl_align = (struct hisi_sgl *)ADDR_ALIGN_64(base + sgl_size * j);
			...
			sgl_pool->sgl_align[num] = sgl_align;
			if (++num == sgl_num)
				break;
		}
	}
...
err_out:
	if (sgl_pool->blks) {
		for (i = 0; i < sgl_pool->blk_num; i++)
			wd_blkpool_free(p, sgl_pool->blks[i]);
		free(sgl_pool->blks);
	}
	if (sgl_pool->sgl_align)
